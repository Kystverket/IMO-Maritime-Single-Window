...
resource "azurerm_container_app" "frontend" {
  ...
  template {
    container {
      name   = "frontend"
      image  = "${data.azurerm_container_registry.acr.login_server}/client:759f01feba341e7af838a77fc140a0302b8e5050"
      cpu    = 0.25
      memory = "0.5Gi"
      env {
        name  = "BACKEND_URL"
...
# 
  template {
    container {
      name   = "backend"
      image  = "${data.azurerm_container_registry.acr.login_server}/server:759f01feba341e7af838a77fc140a0302b8e5050"
      cpu    = 0.25
      memory = "0.5Gi"
      env {
        name  = "PGHOST"
