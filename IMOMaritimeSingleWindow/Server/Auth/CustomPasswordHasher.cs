...
namespace IMOMaritimeSingleWindow.Auth
{
    public class CustomPasswordHasher : PasswordHasher<ApplicationUser>
    {
         ...
         * This is a derived class of https://github.com/aspnet/Identity/blob/dev/src/Core/PasswordHasher.cs
         * The only difference is that it ignores the compatibility options from the default implemenentation
         * and also has increased the iteration count.
 
         * Based on version 3 with increased iteration count:
        ...
        public override string HashPassword(ApplicationUser user, string password)
        {
            ...
            return Convert.ToBase64String(CustomHashPassword(password, _rng));
            
        }

        private byte[] CustomHashPassword(string password, RandomNumberGenerator rng)
        {
            return CustomHashPassword(password, rng,
        ...
        private byte[] CustomHashPassword(string password, RandomNumberGenerator rng, KeyDerivationPrf prf, int iterCount, int saltSize, int numBytesRequested)
        {
