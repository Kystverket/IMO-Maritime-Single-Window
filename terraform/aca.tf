...
resource "azurerm_container_app" "frontend" {
  ...
  template {
    container {
      name   = "frontend"
      image  = "${data.azurerm_container_registry.acr.login_server}/client:9d7a09cc2030cfb8b1e8b986ee461e8d48baad96"
      cpu    = 1.0
      memory = "2Gi"
      env {
        name  = "BACKEND_URL"
...
resource "azurerm_container_app" "backend" {
  ...
  template {
    container {
      name   = "backend" 
      image  = "${data.azurerm_container_registry.acr.login_server}/server:9d7a09cc2030cfb8b1e8b986ee461e8d48baad96"
      cpu    = 0.5
      memory = "1Gi"
      env {
        name  = "PGHOST"
